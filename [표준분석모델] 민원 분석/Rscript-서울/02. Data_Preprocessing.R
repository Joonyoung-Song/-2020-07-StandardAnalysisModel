...
#######################################기존 코드###############################################
# 동의어 처리
sentence <- stemming_words(text = sentence, pattern = pattern_list, replacement = replacement_list)

# 불용어 처리
# sentence <- remove_stopwords(text = sentence, stopword = stopwords_list)

# Morph Analysis
# ncores <- detectCores()		# Core수 확인
# cl <- makeCluster(ncores)	# 클러스터 시작
# doParallel::registerDoParallel(cl)
# system.time({
#    sentence_term <- foreach(i=1:length(sentence), .packages="KoNLP", .combine=c) %dopar% {
#      list(ext_morph(sentence[i], morph="noun", morph2=NULL, morph3=NULL, min.char=2, max.char=10, stopwords=F))
#  
#  })
# 클러스터 종료(필수!)
#  stopCluster(cl)
...
nouns<-sapply(nouns, function(x) gsub("[[:digit:]]","",x))
nouns<-sapply(nouns, function(x) gsub("[[:blank:]]","",x))
nouns<-sapply(nouns, function(x) gsub("[[:space:]]","",x))
nouns<-sapply(nouns, function(x) gsub("[[:punct:]]","",x))
nouns<-sapply(nouns, function(x) gsub("\\d+","",x))
nouns<-sapply(nouns, function(x) gsub("\\(,+","",x))
nouns<-sapply(nouns, function(x) gsub("<,*?>>","",x))
nouns<-sapply(nouns, function(x) gsub("[[]~!@#$%^&*()_+=?><,./]","",x))
nouns<-sapply(nouns, function(x) gsub("[A-Za-z]","",x))
nouns<-sapply(nouns, function(x) Filter(function(x){nchar(x)},x))
...
#############################################################
# 3. 명사 추출
# KoNLP의 extracNoun 함수를 활용하여 명사 추출
...
nouns=sapply(nouns, function(x) gsub("서울","",x))
